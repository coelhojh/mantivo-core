...
import { Database, Check, Copy, ArrowRight, Save, Key, Globe, Info, ShieldAlert, X } from 'lucide-react';
import { saveSupabaseConfig, getActiveConfig } from '../services/supabaseClient';
...
const SetupDatabase: React.FC<SetupDatabaseProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    if (active?.url && (active as any)?.key) {
      setUrl(active.url);
      setKeyValue((active as any).key);
    }
  }, []);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_condos_cnpj ON public.condos (cnpj)
...
for all using (
  ...
  OR owner_id in (select id from public.profiles where account_id = public.get_my_account_id())
);
...
`;
  ...
  const handleSaveConfig = () => {
    if (url && keyValue) {
      saveSupabaseConfig(url, keyValue);
    } else if (onBack) {
      onBack();
    } else {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden border border-slate-200">
        <div className="bg-slate-900 p-6 text-white flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Gerencie a conexão e estrutura do seu Supabase.</p>
          </div>
        ...
        {step === 2 && (
          <div className="p-8 space-y-6">
            <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-200 mb-4">
              <h2 className="text-lg font-bold text-indigo-800 mb-1 flex items-center gap-2">
                <ShieldAlert size={18} /> Estrutura SQL
              </h2>
